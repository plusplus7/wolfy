...
func NewMaimaiStorageFromPackageWithCache(filePath string, aliasPath string, cachePath string) (*MaimaiStorage, error) {
	fromPackage, err := collectSongInfoFromPackage(filePath, aliasPath)
	if err != nil {
		return nil, err
	}
	if cachePath != "" {
		if err := dumpMaimaiStorageCache(fromPackage, cachePath); err != nil {
			return nil, err
		}
	}
	return fromPackage, nil
}
